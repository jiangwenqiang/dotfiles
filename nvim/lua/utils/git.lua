...
local function safe_deep_fetch()
  ...
  if ret ~= 0 then
    Log:error(fmt "Git fetch %s failed! Please pull the changes manually in %s", fetch_mode, get_base_dir())
    return
  end
...
function M.update_base_lvim()
  ...
  if not vim.loop.fs_access(get_base_dir(), "w") then
    Log:warn(fmt("Lunarvim update aborted! cannot write to %s", get_base_dir()))
    return
  end
  ...
  if ret ~= 0 then
    Log:error("Update failed! Please pull the changes manually in " .. get_base_dir())
    return
  end
