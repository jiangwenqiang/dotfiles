...
function plugin_loader.init(opts)
  ...
  if not utils.is_directory(lazy_install_dir) then
    ...
    local core_plugins_dir = join_paths(get_base_dir(), "plugins")
    if utils.is_directory(core_plugins_dir) then
      ...
      local default_snapshot_path = join_paths(get_base_dir(), "snapshots", "default.json")
    ...
    -- load lsp when LazyDone
    vim.api.nvim_create_autocmd("User", { pattern = "LazyDone", callback = require("lsp").setup })
  end

  -- insert lazy_install_dir & plugins_dir
  local rtp = vim.opt.rtp:get()
  table.insert(rtp, #rtp + 1, lazy_install_dir)
  table.insert(rtp, #rtp + 1, join_paths(plugins_dir, "*"))
  vim.opt.rtp = rtp
